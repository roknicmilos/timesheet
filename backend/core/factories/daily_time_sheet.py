...
class DailyTimeSheetFactory(AbstractFactory):
    ...
    @classmethod
    def create(cls, time_sheet_report_count: int = 0, should_store_in_db: bool = None, **kwargs):
        daily_time_sheet = super(DailyTimeSheetFactory, cls).create(
            time_sheet_report_count=time_sheet_report_count,
            should_store_in_db=should_store_in_db,
            **kwargs
        )
    ...
    @classmethod
    def create_batch(cls, count: int, time_sheet_report_count: int = 0, should_store_in_db: bool = None, **kwargs):
        daily_time_sheets = super(DailyTimeSheetFactory, cls).create_batch(
            count=count,
            should_store_in_db=should_store_in_db,
            **kwargs
        )
